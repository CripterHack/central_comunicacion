...
(function() {
    ...
    function initHeader() {
        ...
        for(var i in points) {
            var c = new Circle(points[i], 2+Math.random()*2, 'rgba(0, 0, 0, 1)');
            points[i].circle = c;
        }
    }
    ...
    function shiftPoint(p) {
        TweenLite.to(p, 1+1*Math.random(), {x:p.originX-50+Math.random()*70,
            y: p.originY-70+Math.random()*70, easeInOut:CircInOut,
            onComplete: function() {
                shiftPoint(p);
            }});
    }
